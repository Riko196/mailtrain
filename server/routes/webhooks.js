            ...
            if (req.body.Message) {
                ...
                if (req.body.Message.mail && req.body.Message.mail.messageId) {
                    ...
                    switch (req.body.Message.notificationType) {
                        case 'Bounce':
                            await campaigns.changeStatusByMessage(contextHelpers.getAdminContext(), message, MessageStatus.BOUNCED, req.body.Message.bounce.bounceType === 'Permanent');
                            log.verbose('AWS', 'Marked message %s as bounced', req.body.Message.mail.messageId);
                            break;

                        case 'Complaint':
                            if (req.body.Message.complaint) {
                                await campaigns.changeStatusByMessage(contextHelpers.getAdminContext(), message, MessageStatus.COMPLAINED, true);
    ...
    for (const curEvent of events) {
        ...
        switch (evt.type) {
            case 'bounce':
                ...
                await campaigns.changeStatusByMessage(contextHelpers.getAdminContext(), message, MessageStatus.BOUNCED, [1, 10, 25, 30, 50].indexOf(Number(evt.bounce_class)) >= 0);
            ...
            case 'link_unsubscribe':
                await campaigns.changeStatusByMessage(contextHelpers.getAdminContext(), message, MessageStatus.UNSUBSCRIBED, true);
...
router.postAsync('/sendgrid', async (req, res) => {
    ...
    for (const evt of events) {
        ...
        switch (evt.event) {
            case 'bounce':
                // https://support.sparkpost.com/customer/portal/articles/1929896
                await campaigns.changeStatusByMessage(contextHelpers.getAdminContext(), message, MessageStatus.BOUNCED, true);
    ...
    switch (req.body.event) {
        ...
        case 'MessageDeliveryFailed':
            if (req.body.payload.message && req.body.payload.message.message_id) {
                ...
                if (message) {
                    await campaigns.changeStatusByMessage(contextHelpers.getAdminContext(), message, MessageStatus.BOUNCED, req.body.payload.status === 'HardFail');
